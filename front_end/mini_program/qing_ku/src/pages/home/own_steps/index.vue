  ...
  methods: {
	...
	get_personal_steps_details () {
		...
		wx.getStorage({
			key: 'token',
			success (res) {
				that.$store.dispatch("personal_steps_details", {type: 1, p: 1, token: res.data}).then( (res) => {
					console.log('个人步数领取明细', res);
	...
	get_exchart_steps () {
		...
		wx.getStorage({
			key: 'token',
			success (res) {
				that.$store.dispatch("personal_steps", {p: 1, token: res.data}).then( (res) => {
					console.log('柱形图个人步数', res);
					if (res.data.status == 1) {
						var tem_arr = [];
						var tem_date_list = [];
