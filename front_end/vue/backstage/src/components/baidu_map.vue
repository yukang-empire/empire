...
export default class baidu_map extends Vue{
    ...
    mounted () {
        this.initMap();
        // console.log(this.$store.state.map_data);
        // var that: any = this;
        // var map = new BMap.Map(that.$refs.allmap);
        // map.centerAndZoom(this.$store.state.map_data, 20);
        ...
        loadBMap(key).then(() => {
            ...
            if (this.$store.state.map_data) {
                ...
                if (sessionStorage.getItem('add_business')) {
                    var data = JSON.parse(sessionStorage.getItem('add_business'));
