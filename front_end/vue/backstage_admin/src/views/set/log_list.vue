...
export default class log_list extends Vue{
    //表格、页码数据
    private table_data: any = {
        ...
        page: {
            ...
            is_page: true,
            //当前页码
            current_page: sessionStorage.getItem("log_list_page") ? parseInt(sessionStorage.getItem("log_list_page")) : 1,
            //每页显示的数量
            size: sessionStorage.getItem("log_list_size") ? parseInt(sessionStorage.getItem("log_list_size")) : 10,
            sizes: [10, 15, 20],
            //总数量
            total: 0,
        }
    ...
    private send_data: any = {
        page: sessionStorage.getItem("log_list_page") ? sessionStorage.getItem("log_list_page") : 1,
        size: sessionStorage.getItem("log_list_size") ? sessionStorage.getItem("log_list_size") : 10,
        where: {
            keyword: sessionStorage.getItem("log_list_keyword") ? sessionStorage.getItem("log_list_keyword") : "",
            stime: sessionStorage.getItem("log_list_stime") ? sessionStorage.getItem("log_list_stime") : "",
            etime: sessionStorage.getItem("log_list_etime") ? sessionStorage.getItem("log_list_etime") : ""
        }
    };
    //需要展示的筛选功能
    ...
    //请求log_list数据
    log_list () {
        this.$store.dispatch("log_list", this.send_data).then( (res: any) => {
            console.log("操作列表", res);
            if (res.code == 0) {
    ...
    search (val: any) {
        ...
        sessionStorage.setItem("log_list_page", '1');
        this.send_data.where.keyword = val;
        sessionStorage.setItem("log_list_keyword", val);
        this.log_list();
    };
    ...
    clear_search () {
        ...
        sessionStorage.setItem("log_list_keyword", "");
        this.log_list();
    };
    ...
    change_time(val: any) {
        ...
        sessionStorage.setItem("log_list_stime", this.send_data.where.stime);
        sessionStorage.setItem("log_list_etime", this.send_data.where.etime);
        this.log_list();
    };
    ...
    change_page(val: any) {
        ...
        sessionStorage.setItem("log_list_page", val);
        this.log_list();
    };
    ...
    change_page_size(val: any) {
        ...
        sessionStorage.setItem("log_list_size", val);
        this.log_list();
    };
