<template>
	<div class="p_c_a_s">
		...
		<el-select v-model="form.district" filterable placeholder="请选择区" class="pcas" @change='change_a'>
			<el-option
...
//省-市-区-街道数据
import pcas from '@/assets/api/pcas.js';
...
export default class p_c_a_s extends Vue{
	...
	mounted () {
		...
		if (sessionStorage.getItem('address_data')) {
			this.form = JSON.parse(sessionStorage.getItem('address_data'));
		};
		...
		if (this.form.district) {
			...
			for (var i = 0; i < a_length; i ++) {
				if (this.pcas.a[i].name == this.form.district) {
	...
	change_a () {
		...
		this.$emit("change_a", this.form.district);
	};
	//选择街道
	change_s () {
