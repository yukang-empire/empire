    ...
    methods: {
        ...
        login (name) {
            ...
            if (!is_RE) {
                ...
                params.append("username",login_para.username);
                params.append("password",login_para.password);
                this.$axios.post( that.$store.state.domain + "/index.php?m=Api&c=Club&a=login", params,
                    ).then( function (response) {
                        console.log(response);
                        ...
                        if (res.status == 1) {
                            ...
                            //临时存储后端返回的token和角色
                            sessionStorage.setItem('token', res.result.user_sn);
                            sessionStorage.setItem('role', 1);
                            sessionStorage.setItem('RoleName', '管理员');
