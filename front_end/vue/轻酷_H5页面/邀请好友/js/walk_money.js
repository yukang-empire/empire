        ...
        toOpenVip: function () {
            ...
            if (this.device_rele.is_ios) {
                this.setupWebViewJavascriptBridge( function(bridge) {
                    //toOpenVip为事先与ios开发人员约好的函数名 data为发送过去的数据 response为ios那边返回的数据
                    bridge.callHandler('toOpenVip', data, function(response) {
            ...
            //android终端
                ...
                //toOpenVip为事先与android终端开发人员约好的函数名 data为发送过去的数据(一般建议转成JSON.stringify字符串)
                // var data_ = JSON.stringify(data);
                // window.jsInterface.toOpenVip(data_);
                //把安卓返回的数据装起来
                var res = window.jsInterface.toOpenVip();
