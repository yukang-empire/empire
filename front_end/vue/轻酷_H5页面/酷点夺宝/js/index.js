...
new Vue({
  ...
  methods: {
	...
	pay_db_01 (id, goods_name, goods_points, total_num, goods_price, goods_image) {
		this.buy_dialog = true;
		sessionStorage.setItem('id', id),
		this.goods_name_text = goods_name;
		this.goods_price_text = goods_price;
		this.goods_image_text = 'https://shop.technologyle.com' + goods_image;
		this.kd_need = parseInt(goods_points / total_num);
	},
    // 混合开发的h5与原生APP交互
    pay_db_02 () {
	  var that = this;
	...
	select_phase (index) {
		...
		console.log(this.newest_phase);
		if (this.newest_phase) {
			
			that.newest_phase_win_num = this.newest_phase.luck_list;
			if (that.newest_phase_win_num.length > 0) {
				
				for (var j = 0;j<that.newest_phase_win_num.length;j++) {
					if (that.newest_phase_win_num[j].luck_no == this.newest_phase.give_no) {
						that.is_win = true;
					};
				};
			};
			that.progress = (that.newest_phase.join_num / that.newest_phase.total_num) * 100;
          ...
          success (response) {
              ...
              if (response.status == 1) {
				...
				for (var i = 0; i < length; i++) {
					if (that.home_data[i].status == 1) {
						that.home_data[i].status = '进行中';
					}else if (that.home_data[i].status == 2) {
						that.home_data[i].status = '已完成';
					}else if (that.home_data[i].status == 3) {
						that.home_data[i].status = '已开奖';
					};
				};
				that.newest_phase = that.home_data[0];
				that.newest_phase_win_num = that.home_data[0].luck_list;
				that.progress = (that.newest_phase.join_num / that.newest_phase.total_num) * 100;
				if (that.newest_phase_win_num.length>0) {
					
					for (var j = 0;j<that.newest_phase_win_num.length;j++) {
						if (that.newest_phase_win_num[j].luck_no == that.newest_phase.give_no) {
							that.is_win = true;
						}
					};
				};
